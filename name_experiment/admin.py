                ...
                for data in contents2:
                    ...
                    if c > 1:
                        ...
                        if count == 0:
                            ...
                            if row[0].strip() == "":
                                messages.error(
                                    request, "One black row. Not is possible load it")
                            else:
                                ActiveExperiments.objects.create(
                                    ...
                                    test=row[2].strip(),
                                    experiment=row[3].strip(),
                                    namespace=row[4].strip(),
                                    is_active=True
                                )

                                messages.success(
                                    request, "Upload Experiment: " + stringMessage)

                        else:
                            messages.warning(
                                request, "Experiment already exist: " + stringMessage)

        else:
            print('File not exist')
            messages.error(request, "File not exist: " +
                           obj.name_experiments_file)


class NameExperimentAdmin(admin.ModelAdmin):

    list_display = ('name_experiments_file', 'is_active',
                    'is_running', 'is_finished', 'is_active')
    list_filter = ('name_experiments_file', 'is_active',
                   'is_running', 'is_finished', 'is_active')
    search_fields = ('name_experiments_file', 'is_active',
                     'is_running', 'is_finished', 'is_active')
    ordering = ('name_experiments_file', 'is_active',
                'is_running', 'is_finished', 'is_active')
    filter_horizontal = ()
    list_per_page = 25
